...
export class ListTaskComponent implements OnInit {
  ...
  deleteTaskFront(task: any){
    let index;
        if(task.taskStatus === 'to-do'){
          index = this.taskTodo.indexOf(task);
          if (index > -1) {
            this.taskTodo.splice(index, 1);
          }
        }
        if(task.taskStatus === 'in-progress'){
          index = this.taskInprogress.indexOf(task);
          if (index > -1) {
            this.taskInprogress.splice(index, 1);
          }
        }
        if(task.taskStatus === 'done'){
          index = this.taskDone.indexOf(task);
          if (index > -1) {
